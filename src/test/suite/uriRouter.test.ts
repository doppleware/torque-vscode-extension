...
suite('UriRouter Test Suite', () => {
	...
	test('Should handle URI with query parameters', async () => {
		...
		const testUri = vscode.Uri.parse('vscode://Quali.torque/test/query?param1=value1&param2=value2');
		
		// Should not throw
		await uriRouter.handleUri(testUri);
		
		assert.ok(true, 'URI with query parameters should be handled without errors');
	});
});