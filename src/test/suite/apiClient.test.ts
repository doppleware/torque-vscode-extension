...
suite('ApiClient Test Suite', () => {
	...
	test('Should update config', () => {
		...
		assert.strictEqual(apiClient.client.defaults.headers['X-API-Token'], `Token ${newToken}`);
	});
	...
	test('Should clear session', () => {
		...
		assert.strictEqual(apiClient.client.defaults.headers.authorization, undefined);
	});
});